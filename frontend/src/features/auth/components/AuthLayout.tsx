...
function Brand() {
  return (
    <div className="flex flex-col items-center text-center">
      <svg aria-hidden="true" viewBox="0 0 24 24" className="size-10 fill-none stroke-primary-action stroke-[1.5]">
      ...
      <span className="mt-xs font-display text-display-sm text-ink">OralCardio</span>
    </div>
  );
}
...
export function AuthLayout({ mode, title, subtitle, children, footer }: AuthLayoutProps) {
  ...
  return (
    <main className="min-h-dvh bg-canvas min-[1024px]:m-xs min-[1024px]:grid min-[1024px]:min-h-[calc(100dvh-0.5rem)] min-[1024px]:grid-cols-[minmax(17rem,0.65fr)_minmax(0,1.35fr)] min-[1024px]:overflow-hidden min-[1024px]:rounded-lg min-[1024px]:border min-[1024px]:border-hairline-soft">
      <aside className="relative hidden overflow-hidden bg-surface-soft px-xxl py-xxl min-[1024px]:flex min-[1024px]:items-center">
        <div className="flex h-full w-full max-w-[24rem] flex-col items-center justify-between text-center">
          <div>
            <Brand />
            <p className="mt-sm max-w-[15rem] font-body text-body-sm text-muted">{mode === "sign-in" ? "Cuidar do seu sorriso é cuidar do seu coração." : "Comece hoje a cuidar da sua saúde bucal e cardíaca."}</p>
            <div className="mt-sm flex w-20 items-center gap-xs text-primary-action" aria-hidden="true">
