...
class AchievementRevealOutput(BaseModel):
    id: UUID
    name: str
    description: str
    icon: str
    ...
    @classmethod
    def from_record(cls, record: AchievementRecord) -> "AchievementRevealOutput":
        return cls(
            id=record.id,
            name=record.name,
...
class AchievementRevealAcknowledgeInput(BaseModel):
    achievement_ids: list[UUID] = Field(min_length=1, max_length=100)
